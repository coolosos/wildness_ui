...
class DeviceScenario {
  ...
  DeviceScenario({required this.key, required this.widget, this.onCreate});
...
class DeviceBuilder {
  ...
  DeviceBuilder({this.wrap, this.bgColor});
  ...
  List<TestDevice> _devicesForScenarios = Devices.common;
  ...
  void overrideDevicesForAllScenarios({required List<TestDevice> devices}) {
    assert(
      devices.isNotEmpty,
      'At least one device must be provided to overrideDevicesForAllScenarios',
    );
  ...
  Widget build() {
    ...
    assert(
      scenarios.length % numberOfDevices == 0,
    ...
    return Align(
      ...
      child: Container(
        ...
        child: Column(children: [...scenarioRows]),
      ),
    );
  }
}
...
class DeviceScenarioWidget extends StatelessWidget {
  ...
  static const double _horizontalScenarioPadding = 8;
    ...
    return ClipRect(
      child: Padding(
        padding: const EdgeInsets.symmetric(
          horizontal: _horizontalScenarioPadding,
        ),
        child: DecoratedBox(
